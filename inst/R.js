...
function subset_vector_by_boolean(vec0, bool0) {
  var res = [];
  for (var i = 0; i < bool0.length; i++) {
    if (bool0[i] == true) {
      res.push(vec0[i]);
    }
  }
...
function subset_list_by_boolean(list0, bool0) {
  var key_list = Object.keys(list0);
  for (var i = 0; i < key_list.length; i++) {
    key = key_list[i];
    list0[key] = subset_vector_by_boolean(list0[key], bool0);
  }
  return list0;
}

function subset_array_by_boolean(arr0, bool0) {
  for (var i = 0; i < arr0.length; i++) {
    arr0[i] = subset_vector_by_boolean(arr0[i], bool0);
  }
  return arr0;
}

function unique(arr0) {
  var result = Array.from(new Set(arr0));
  return result;
}
