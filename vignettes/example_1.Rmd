---
title: 'Tutorial 1: Javascript-R communication mechanism'
author: "Jackson Kwok"
date: "`r Sys.Date()`"
output:
...
1. `ws.send(str0)`: this command lives in JS, and it sends a string to R every time it's called. To send complex data, the standard trick is to use `ws.send(JSON.stringify(my_complex_data))`.

2. `ws.onmessage(msg) = function { ... }`: this function lives in JS. It continuously monitors if R has sent JS a message, and it runs the code in the body when it gets a message. The message contains other metadata, and we use `JSON.parse(msg.data)` to extract the data from the message.
...
The following is a common pattern to use. It is very flexible; in fact, all examples are created under this framework. More 'custom' pattern can be created, but we shall leave that to another tutorial.
...
- JavaScript
```
ws.send("JSON.stringify({x:3, y:4})");
ws.onmessage = function(msg) {
  var r_data = JSON.parse(msg.data);
...
- for building the html interface, we have `add_title, add_slider, add_div`; 

- for JavaScript, we have `document.getElementById('_ID_')`;

- for running the app, we have `write_html_to_file(), create_app(), start_app()`.
